...
function Menu({ show, onClose }) {
    ...
    return (
        <div className={`backdrop-blur-xl w-[70%] fixed z-30 h-full bg-[rgba(0,0,0,0.8)] top-0 rounded-l-3xl py-5 ${show ? 'show right-0' : 'hide -right-full'} `}>

            <div className="flex between v-center px-5 my-3">
                <img src="logo.png" className="h-10" alt="" />
                <i className="fab fa-github text-2xl px-1 border border-white/60 rounded-full"></i>
            </div>

            <div className="line5 w90 grey"></div>
...
function Button({ icon, text, active, onClick, to }) {
    return (
        <Link
            onClick={onClick}
            to={to}
            className={`hblue-sm flex px-3 py-1 rounded-lg v-center gap-5 my-3 ${active ? 'blue' : ''}`}
        >
            <div className="flex center w-3 text-lg">
                <i className={`${active ? 'fas' : 'far'} ${icon} p-0 m-0`}></i>
            </div>
